...
function Recorder(window) {
    ...
    this.registerTopWindowListener();
}
...
//CloseWindowExt, Syuan-Yi Lin, SELAB, CSIE, NCKU, 2016/11/23
Recorder.prototype.registerTopWindowListener = function() {
	if (this.window === this.window.top) {
		...
		this.window.addEventListener("load", function() {
			...
			if(this.window.location.href != "about:blank")
				self.record("pseudoLoad", this.window.name);
		}, false);
		this.window.addEventListener("unload", function() {
			//CloseWindowExt, Syuan-Yi Lin, SELAB, CSIE, NCKU, 2016/12/09
			if(this.window.location.href != "about:blank")
				self.record("pseudoUnload", this.window.name);
		}, false);
	}
}
