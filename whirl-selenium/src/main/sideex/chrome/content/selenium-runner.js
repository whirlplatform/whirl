...
objectExtend(IDETestLoop.prototype, {
  ...
  _executeCurrentCommand : function() {
    ...
    if (executeUsingWebDriver) {
      ...
      this.showNotFoundLog = false;
      this.implicitTime = "";
      var implicitWait = function() {
        try {
          ...
          this.elementNotFound = match_not_found && !failed_locator;

          // check timed out or not
          if (this.implicitTime && (Date.now() - this.implicitTime) > 30000) {
            this.implicitTimedout = true;
            this.elementNotFound = false; 
          ...
          if (this.elementNotFound) {
            if (!this.showNotFoundLog) {
              this.showNotFoundLog = true;
              this.implicitTime = Date.now();
              LOG.info("Sideex: Wait until the element is found");
            }
            setTimeout(implicitWait, 100);
          } else {
            if (failed_locator) {
              ...
              this.waitForCondition = this.result.terminationCondition;
              this.continueTestWhenConditionIsTrue();
            } else if (this.implicitTimedout) {
              this.implicitTimedout = false;
              // print error message and stop testcase
              try {
                this.result = handler.execute(selenium, command);
              } catch (e) {
