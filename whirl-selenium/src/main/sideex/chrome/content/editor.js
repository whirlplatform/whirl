...
function Editor(window) {
  ...
  this.count = 1;
  //CloseWindowExt, Syuan-Yi Lin, SELAB, CSIE, NCKU, 2016/11/23
  this.windowStatus = {};
  Object.defineProperty(this.windowStatus, "", {value : true, writable : true ,
                                                enumerable: false, configurable: false});
  this.lastClickDate = new Date();
  //CloseWindowExt, Syuan-Yi Lin, SELAB, CSIE, NCKU, 2016/11/24
  this.lastUnloadTarget = "";
  this.lastSelectWindowTarget = "";
  //CloseWindowExt, Syuan-Yi Lin, SELAB, CSIE, NCKU, 2016/11/27
  this.lastUnloadWindow = window;
  this.unloadPriority = 0;
  this.checkClosedWindowEnable = false;
  ...
  if (!this.app.options.currentVersion || this.app.options.currentVersion != versionString) {
    // change the website to sideex's official site, Yu-Xian Chen, SELAB, CSIE, NCKU, 2016/11/07
    openTabOrWindow('http://sideex.org/');
  ...
  //UnnamedWinIFrameExt, Jie-Lin You, SELAB, CSIE, NCKU, 2016/12/02
  this.selDebugger.editor.toggleRecordingEnabled(false);
}
...
Editor.prototype.toggleRecordingEnabled = function (enabled) {
  ...
  //CloseWindowExt, Syuan-Yi Lin, SELAB, CSIE, NCKU, 2016/12/09
  if (enabled === false && this.checkClosedWindowEnable) {
    this.checkClosedWindowEnable = false;

    //CloseWindowExt, Syuan-Yi Lin, SELAB, CSIE, NCKU, 2016/12/19
    if (!Components.utils.isDeadWrapper(this.lastUnloadWindow))
      this.windowStatus[this.lastUnloadTarget] = true;

    var unloadArray = [];
    for (var windowName in this.windowStatus)
      if (this.windowStatus[windowName] !== true/* && windowName != window.top.name*/) {
        unloadArray.push({name:windowName, priority:this.windowStatus[windowName]});
        this.windowStatus[windowName] = true;
      }

    unloadArray.sort(function(a,b) {return a.priority - b.priority;});
   
    for (var i = 0; i < unloadArray.length; i++) {
      if (this.lastSelectWindowTarget != "name=" + unloadArray[i]["name"])
        this.addCommand('selectWindow', "name=" + unloadArray[i]["name"], '', "WINDOW CLOSED");
      this.addCommand('close', '', '', "WINDOW CLOSED");
      //force setWindow() to reset this.topName
      this.topName = unloadArray[i]["name"];
    }
    /*
    if (unloadArray.length != 0 && command != 'selectWindow')
      this.addCommand('selectWindow', "name=" + window.top.name, '', window);
    */
  }
...
Editor.prototype.onUnloadDocument = function (doc) {
  ...
  //AJAXWaitExt & PageLoadingWaitExt, Yu-Xian Chen, SELAB, CSIE, NCKU, 2016/10/7
  // do not record 'AndWait' suffix, instead, automatically use SideeX's wait extenstion
...
Editor.prototype.addCommand = function (command, target, value, window, insertBeforeLastCommand) {
  ...
  this.log.debug("addCommand: command=" + command + ", target=" + target + ", value=" + value 
                 + " window.name=" + (window === "WINDOW CLOSED" ? "" : window.name));
  if (command != 'open' &&
      command != 'selectWindow' &&
      command != 'selectFrame' &&
      command != 'close' &&
      (command != 'pseudoLoad' || target == "") &&
      command != 'pseudoUnload') {
    if (this.getTestCase().commands.length == 0) {
	  //UnnamedWinIFrameExt, Jie-Lin You, SELAB, CSIE, NCKU, 2016/07/05
	  this.count = 1;
	  //UnnamedWinIFrameExt, Jie-Lin You, SELAB, CSIE, NCKU, 2016/07/19
	  window.top.name = "win_ser_local"; 
      var top = this._getTopWindow(window);
      this.log.debug("top=" + top);
      var path = this.getPathAndUpdateBaseURL(top)[0];
      //CloseWindowExt, Syuan-Yi Lin, SELAB, CSIE, NCKU, 2016/11/23
      this.windowStatus[window.top.name] = true;
      this.addCommand("open", path, '', top);
      this.recordTitle(top);
    }
	//UnnamedWinIFrameExt, Jie-Lin You, SELAB, CSIE, NCKU, 2016/08/02
	if (!this.safeLastWindow.adjustWindow(window)) {
	  if (window.top.name == '') {//popup
		window.top.name = "win_ser_" + this.count;	
		this.count += 1;
      }
    //CloseWindowExt, Syuan-Yi Lin, SELAB, CSIE, NCKU, 2016/11/23
    this.windowStatus[window.top.name] = true;
	  this.addCommand('selectWindow', "name=" + window.top.name, '', window);
	  this.safeLastWindow.setWindow(window.top);
	} else if (window.name == '') {//frame //UnnamedWinIFrameExt, Jie-Lin You, SELAB, CSIE, NCKU, 2016/07/27
	  this.safeLastWindow.setWindow(window.top);
	}
	//UnnamedWinIFrameExt, Jie-Lin You, SELAB, CSIE, NCKU, 2016/11/24
    if (!this.safeLastWindow.windowEquals(window)) {
      if (this.safeLastWindow.isSameTopWindow(window)) {
        ...
        if (windowName == '') {
          ...
          //CloseWindowExt, Syuan-Yi Lin, SELAB, CSIE, NCKU, 2016/11/23	
		  this.windowStatus[window.name] = true;
          this.count += 1;
    ...
    //CloseWindowExt, Syuan-Yi Lin, SELAB, CSIE, NCKU, 2016/11/27
    else if (!Components.utils.isDeadWrapper(this.lastUnloadWindow))
      this.windowStatus[this.lastUnloadTarget] = true;
  }

  //CloseWindowExt, Syuan-Yi Lin, SELAB, CSIE, NCKU, 2016/11/27
  if (command == 'pseudoLoad') {
    if (this.safeLastWindow.windowEquals(window))
      this.windowStatus[this.lastUnloadTarget] = true;
    this.windowStatus[target] = true;
    return;
  } else if (command == 'pseudoUnload') {
    this.lastUnloadTarget = target;
    this.lastUnloadWindow = window;
    if (new Date() - this.lastClickDate <= 300)
    {
      this.windowStatus[target] = true;
      return;
    }
      ...
      //CloseWindowExt, Syuan-Yi Lin, SELAB, CSIE, NCKU, 2016/12/09
      this.checkClosedWindowEnable = true;
      this.windowStatus[target] = this.unloadPriority++;
      return;
    }
  }

  //check if any window had closed before adding next command
  //CloseWindowExt, Syuan-Yi Lin, SELAB, CSIE, NCKU, 2016/12/09
  if (this.checkClosedWindowEnable && window === window.top) {
    this.checkClosedWindowEnable = false;
    var unloadArray = [];
    for (var windowName in this.windowStatus)
      if (this.windowStatus[windowName] !== true && windowName != window.top.name) {
  ...
  if (insertBeforeLastCommand && this.view.getRecordIndex() > 0) {
    ...
    if(command.command == 'doubleClick' && this.lastCommandIndex >= 2){
      var l1 = this.getTestCase().commands[this.lastCommandIndex - 1];
      var l2 = this.getTestCase().commands[this.lastCommandIndex - 2];
      if(l1.command == l2.command && (l1.command == 'click' || l1.command == 'clickAt') && l1.target == l2.target && l1.target == command.target){
        if(l1.command == 'clickAt') command = new Command('doubleClickAt', target, l1.value);
        this.lastCommandIndex -= 2;
        this.getTestCase().commands.splice(this.lastCommandIndex, 2, command);
        this.view.rowDeleted(this.lastCommandIndex + 1);
      }
    }else{
  ...
  if (command.command == 'click')
    this.lastClickDate = new Date();
  //CloseWindowExt, Syuan-Yi Lin, SELAB, CSIE, NCKU, 2016/11/25
  else if (command.command == 'selectWindow')
    this.lastSelectWindowTarget = target;
};
...
if (lastCommand.type == 'command' && !lastCommand.command.match(/^(assert|verify|store|mouseOver)/)) {
...
Editor.prototype.playCurrentTestCase = function (next, index, total) {
  ...
  this.selDebugger.start(function (failed) {
    ...
    if (testCase) {
	  ...
	  //AJAXWaitExt & PageLoadingWaitExt, Yu-Xian Chen, SELAB, CSIE, NCKU, 2016/07/05
	  if(self.getUserLog().lastCommandRestitute) {
        self.getUserLog().lastCommandRestitute();
      } 
...
GeneratedReference.prototype.doShow = function (frame) {
  // remove AndWait command from list, Yu-Xian Chen, SELAB, CSIE, NCKU, 2016/11/27
  if (!this.selectedCommand.command.match(/(AndWait)$/)) {
...
function LastWindow() {
  ...
  this.path = [];
  //UnnamedWinIFrameExt, Jie-Lin You, SELAB, CSIE, NCKU, 2016/05/26
  this.checkOnlyFirstWindow = true;
}
...
LastWindow.prototype.isSameTopWindow = function (window) {
  ...
  //UnnamedWinIFrameExt, Jie-Lin You, SELAB, CSIE, NCKU, 2016/11/27
...
//UnnamedWinIFrameExt, Jie-Lin You, SELAB, CSIE, NCKU, 2016/05/26
LastWindow.prototype.adjustWindow = function (window) {	
  if (this.checkOnlyFirstWindow == true){
	this.checkOnlyFirstWindow = false;
	if ( window.top.name=="" ){
	  window.top.name = "win_ser_local";
	  this.topName = window.top.name;
	  this.name = window.name;
  ...
  if (window.name.search("index")==0)
    this.topName = window.top.name;    
	
  if ((window.name != window.top.name && this.topName != window.top.name) || window.top.name == '') {
	this.topName = window.top.name;
	return false;
  }
  return true;
