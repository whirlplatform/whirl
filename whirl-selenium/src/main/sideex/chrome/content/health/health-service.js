...
HealthService.prototype.attach = function(win, component) {
  if (win) {
    ...
    win.onerror = function SIDEErrorHandler(msg, url, line, column, error) {
      ...
      if (self.alerts) {
        try {
          // fix selenium's bug, Yu-Xian Chen, SELAB, CSIE, NCKU, 2016/11/14
          // We found that in ff49, sometimes it will alert testCase.debugContext.currentCommand(...) is undefined
          // but the testCase can correctly finish, so we think it's a bug between selenium 2.9.1 and ff49
          // we can't fix this now, so we temporary use this bad solution
          var currentCommandUndefined = msg.match(/(testCase.debugContext.currentCommand\(...\) is undefined)/);
          if (!currentCommandUndefined) {
