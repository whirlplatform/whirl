 ...
 * <p>
 * There are some options defined here (as static strings), see the codemirror
 * manual for all possible options.
 * 
...
public class Config {
	 ...
	 * names the mode (for example {name: "javascript", json: true}). The demo
	 * pages for each mode contain information about what configuration
	 * parameters the mode supports. You can ask CodeMirror which modes and MIME
	 * types have been defined by inspecting the CodeMirror.modes and
	 * CodeMirror.mimeModes objects. The first maps mode names to their
	 ...
	 * defining the corresponding .cm-s-[name] styles is loaded (see the theme
	 * directory in the distribution). The default is "default", for which
	 * colors are included in codemirror.css. It is possible to use multiple
	 * theming classes at once—for example "foo bar" will assign both the
	 * cm-s-foo and the cm-s-bar classes to the editor.
	 */
	 ...
	 * focused, or it has an autofocus attribute and no other element is
	 * focused.
	 */
	...
	/**
	 * Creates a new empty configuration object, that can be used to create a
	 * codemirror editor.
	 * 
	 ...
	 * <p>
	 * See static strings defined on this class for possible options (note that
	 * this is a non-exclusive list as Codemirror may be extended with extra
	 * plugins which all have their own configuration option.)
	 * 
	 * @param key
	 * @param value
	 */
