 ...
 * <p>
 * There are some options defined here (as static strings), see the codemirror manual for all
 * possible options.
 *
...
public class Config {
    ...
    /**
     * The mode to use. When not given, this will default to the first mode that was loaded. It may
     * be a string, which either simply names the mode or is a MIME type associated with the mode.
     * Alternatively, it may be an object containing configuration options for the mode, with a name
     * property that names the mode (for example {name: "javascript", json: true}). The demo pages
     * for each mode contain information about what configuration parameters the mode supports. You
     * can ask CodeMirror which modes and MIME types have been defined by inspecting the
     * CodeMirror.modes and CodeMirror.mimeModes objects. The first maps mode names to their
     * constructors, and the second maps MIME types to mode specs.
     */
     ...
     * corresponding .cm-s-[name] styles is loaded (see the theme directory in the distribution).
     * The default is "default", for which colors are included in codemirror.css. It is possible to
     * use multiple theming classes at once—for example "foo bar" will assign both the cm-s-foo and
     * the cm-s-bar classes to the editor.
     */
    public static final String THEME = "theme"; // : string

    /**
     * How many spaces a block (whatever that means in the edited language) should be indented. The
     * default is 2.
     */
     ...
     * same as the line before). Defaults to true.
     */
    ...
    /**
     * Whether, when indenting, the first N*tabSize spaces should be replaced by N tabs. Default is
     * false.
     */
    ...
    /**
     * Configures the keymap to use. The default is "default", which is the only keymap defined in
     * codemirror.js itself. Extra keymaps are found in the keymap directory. See the section on
     * keymaps for more information.
     */
    public static final String KEYMAP = "keyMap"; // : string

    /**
     * Can be used to specify extra keybindings for the editor, alongside the ones defined by
     * keyMap. Should be either null, or a valid keymap value.
     */
     ...
     */
    public static final String LINENUMBERFORMATTER = "lineNumberFormatter"; // :
    // function(line:
    // integer)
    // ->
    // string
    ...
    /**
     * Can be used to add extra gutters (beyond or instead of the line number gutter). Should be an
     * array of CSS class names, each of which defines a width (and optionally a background), and
     * which will be used to draw the background of the gutters. May include the
     * CodeMirror-linenumbers class, in order to explicitly set the position of the line number
     * gutter (it will default to be to the right of all other gutters). These class names are the
     * keys passed to setGutterMarker.
     */
     ...
     * it stays fixed during horizontal scrolling (true, the default).
     */
     ...
     */
    public static final String SHOWCURSORWHENSELECTING = "showCursorWhenSelecting"; // :
    // boolean
     ...
     * when typing or deleting. Defaults to 500.
     */
    public static final String HISTORYEVENTDELAY = "historyEventDelay"; // :
    // integer
    ...
    /**
     * Can be used to make CodeMirror focus itself on initialization. Defaults to off. When
     * fromTextArea is used, and no explicit value is given for this option, it will be set to true
     * when either the source textarea is focused, or it has an autofocus attribute and no other
     * element is focused.
     */
    ...
    /**
     * Creates and returns a {@link Config} object usable for CLike data.
     *
     * @return Configuration
     * @see CodeMirrorWrapper
     */
    public static Config forJava() {
        Config config = getDefault();
        config.setOption(MODE, "text/x-java");
        return config;
    }
    ...
    /**
     * Creates and returns a {@link Config} object usable for JavaScript data.
     *
     * @return Configuration
     * @see CodeMirrorWrapper
     */
    public static Config forJavaScript() {
        Config config = getDefault();
        config.setOption(MODE, "javascript");
        return config;
    }

    /**
     * Creates and returns a {@link Config} object usable for SQL data.
     *
     * @return Configuration
     * @see CodeMirrorWrapper
     */
     ...
     * <p>
     * See static strings defined on this class for possible options (note that this is a
     * non-exclusive list as Codemirror may be extended with extra plugins which all have their own
