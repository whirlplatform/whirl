...
public class EditGridToolBar extends ToolBar {
	...
	public EditGridToolBar(ClassMetadata metadata, boolean hideGroups, boolean showData, boolean showSearch,
			boolean showExport, boolean showMethod, boolean showRefresh, LoadConfigProvider configProvider,
			ParameterHelper paramHelper) {
		this(metadata, hideGroups, showData, showSearch, showExport, showMethod, showRefresh, configProvider,
				paramHelper, null);
	...
	private void initData(FlexTable dataTable) {
		if (showData && (metadata.isViewable() || metadata.isInsertable() || metadata.isUpdatable()
			...
			if (metadata.isViewable() && !metadata.isUpdatable()) {
				viewBtn = createButton(AppMessage.Util.MESSAGE.view(), ApplicationBundle.INSTANCE.view());
				if (!hideGroups) {
			...
			if (metadata.isInsertable()) {
				insertBtn = createButton(AppMessage.Util.MESSAGE.insert(), ApplicationBundle.INSTANCE.add());
				copyBtn = createButton(AppMessage.Util.MESSAGE.copy(), ApplicationBundle.INSTANCE.copy());
				if (!hideGroups) {
			...
			if (metadata.isUpdatable()) {
				updateBtn = createButton(AppMessage.Util.MESSAGE.update(), ApplicationBundle.INSTANCE.edit());
				if (!hideGroups) {
			...
			if (metadata.isDeletable()) {
				deleteBtn = createButton(AppMessage.Util.MESSAGE.delete(), ApplicationBundle.INSTANCE.delete());
				if (!hideGroups) {
	...
	private void initSearch(FlexTable groupTable) {
		if (showSearch) {
			// search
			searchBtn = createButton(AppMessage.Util.MESSAGE.filter(), ApplicationBundle.INSTANCE.search());

			// sort
			sortBtn = createButton(AppMessage.Util.MESSAGE.sort(), ApplicationBundle.INSTANCE.sort());
	...
	private void initExport(FlexTable groupTable) {
		if (showExport) {
            ...
            expCsvBtn = exportImport.create(metadata, GridExportImport.ExpImpType.EXPORT_CSV, AppMessage.Util.MESSAGE.expimp_exportCSV(),
					false);
            ...
            impCsvBtn = exportImport.create(metadata, GridExportImport.ExpImpType.IMPORT_CSV, AppMessage.Util.MESSAGE.expimp_importCSV(),
					false);
	...
	private void initRefresh(FlexTable groupTable) {
		if (showRefresh) {
			// refresh
			refreshBtn = createButton(AppMessage.Util.MESSAGE.refresh(), ApplicationBundle.INSTANCE.refresh());
			if (!hideGroups) {
				refreshBtn.setText(AppMessage.Util.MESSAGE.refresh());
				addToGroup(groupTable, refreshBtn);
			} else {
