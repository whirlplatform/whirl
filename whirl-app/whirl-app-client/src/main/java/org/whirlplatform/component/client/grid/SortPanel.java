...
public class SortPanel extends Window implements SortEvent.HasSortHandlers, LocatorAware {
	...
	private void initDualList() {
		...
		ValueProvider<SortValue, SortValue> valueProvider = new ValueProvider<SortValue, SortValue>() {
		...
		dualList = new DualListField<SortValue, SortValue>(fromStore, toStore, valueProvider, new SortRowCell(toStore));
	...
	public void fillSorts(List<SortValue> data) {
		...
		for (SortValue m : data) {
			if (toStore.findModelWithKey(toStore.getKeyProvider().getKey(m)) != null) {
				toStore.findModelWithKey(toStore.getKeyProvider().getKey(m)).setOrder(m.getOrder()); // Восстановление
																										// сортировки
	...
	private Element getFromViewElement(Locator part) {
		...
		Element byLabel = getElementByParam(sortValPart.getParameter(type), type, false, store, view);
		if (byLabel != null) {
			return byLabel;
		}
		type = LocatorParams.PARAM_INDEX;
		Element byIndex = getElementByParam(sortValPart.getParameter(type), type, false, store, view);
		if (byIndex != null) {
			return byIndex;
		}
		return null;
	...
	private Element getToViewElement(Locator part) {
		...
		type = LocatorParams.PARAM_LABEL;
		Element byLabel = getElementByParam(sortValPart.getParameter(type), type, isImg, store, view);
		if (byLabel != null) {
			return byLabel;
		}
		type = LocatorParams.PARAM_INDEX;
		Element byIndex = getElementByParam(sortValPart.getParameter(type), type, isImg, store, view);
		if (byIndex != null) {
			return byIndex;
		}
		return null;
	...
	private Element getElementByParam(String param, String paramType, boolean isImg, ListStore<SortValue> store,
	...
	/**
	 * Пытаемся извлечь изображение, если не удалось возвращаем родительский
	 * элемент.
	 * 
	 * @param element
	 *            - элемент у которого пытаемся получить изображение.
	 * @return
	 */
