...
public class FieldMetadataHelper {
	...
	public static Map<String, DataValue> prepareProperties(FieldMetadata field,
		...
		if (field.getRegEx() != null && !field.getRegEx().isEmpty()) {
			properties.put(PropertyType.RegEx.getCode(), new DataValueImpl(DataType.STRING, field.getRegEx()));
		}
		...
		if (field.getType() == DataType.LIST) {
			properties.put(PropertyType.ReloadStructure.getCode(), new DataValueImpl(DataType.BOOLEAN, true));
		}
		return properties;
	}
