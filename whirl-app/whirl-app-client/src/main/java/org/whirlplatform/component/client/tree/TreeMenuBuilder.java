...
public class TreeMenuBuilder extends TreeBuilder implements ClickEvent.HasClickHandlers,
		Containable {
		...
		iconProvider = new IconProvider<RowModelData>() {
			...
			public ImageResource getIcon(RowModelData model) {
				...
				if (model.getId().startsWith("temp")) {
					...
					if (image != null
							&& BuilderManager.getApplicationData() != null) {
						String role = BuilderManager.getApplicationData()
								.getApplicationCode();
	...
	protected TreeLoader<RowModelData> initLoader(
			final TreeStore<RowModelData> store) {
		...
		TreeLoader<RowModelData> loader = new TreeLoader<RowModelData>(proxy) {
			...
			public boolean hasChildren(RowModelData parent) {
				...
				if (getClassMetadata().getClassId() != null
						&& parent.getProperties().containsKey(
								leafExpressionColumn)) {
					return Boolean.valueOf(parent
							.<Boolean> get(leafExpressionColumn));
				} else {
			...
			@Override
			protected void onLoadSuccess(RowModelData loadConfig,
					List<RowModelData> result) {
			...
			protected void loadData(RowModelData config) {
				if (builderMap.values().contains(config)) {
					...
					if (builder != null) {
						for (ComponentBuilder c : ((HorizontalMenuItemBuilder) builder)
								.getChildren()) {
	...
	@Override
	protected XTree<RowModelData, String> initTree(
			TreeLoader<RowModelData> loader) {
		...
		tree.setSelectionModel(new TreeSelectionModel<RowModelData>() {
			...
			protected void onMouseClick(
					com.google.gwt.event.dom.client.ClickEvent ce) {
		...
		tree.getSelectionModel().addSelectionChangedHandler(
				new SelectionChangedHandler<RowModelData>() {

					@Override
					public void onSelectionChanged(
							SelectionChangedEvent<RowModelData> event) {
						for (Entry<ComponentBuilder, RowModelData> entry : builderMap
								.entrySet()) {
							if (event.getSelection().size() > 0
									&& entry.getValue() == event.getSelection()
											.get(0)) {
		...
		SimpleSafeHtmlCell<String> cell = new SimpleSafeHtmlCell<String>(
			...
			public void onBrowserEvent(Context context, Element parent,
				...
				if (eventColumn != null && "click".equals(event.getType())) {
					...
					RowModelData model = getStore()
							.getChild(context.getIndex());
	...
	protected ClassMetadata getClassMetadata() {
		...
		if (eventColumn != null) {
			metadata.addField(new FieldMetadata(eventColumn, DataType.STRING,
					null));
		}
		return metadata;
	}
	...
	private RowModelData getModelData(HorizontalMenuItemBuilder cb) {
		...
		// }
		model.set(nameExpressionColumn, cb.getTitle());
