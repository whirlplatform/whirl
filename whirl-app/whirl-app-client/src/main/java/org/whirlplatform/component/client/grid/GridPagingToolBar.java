...
class GridPagingToolBar extends ToolBar implements LocatorAware {
	...
	/**
	 * Храню флаги видимости в component.data. Сам этот метод не делает
	 * компонент видимым. Действие происходит позже. Внутри класса
	 * GridPagingToolbar.
	 */
	...
	/**
	 * Управление видимостью посредством условного добавления компонентов к
	 * контейнеру. Вместо стандартного c.setVisible(), которое некорректно
	 * работает если компонент в текущий момент невидим.
	 */
	...
	private Locator getSelectorLocator(Element element, boolean isSelectorChild, boolean isSelectorItem) {
		if (isSelectorChild) {
			...
			} else if (selector.getCell().getAppearance().triggerIsOrHasChild(selector.getElement(), element)) {
				Locator selectorlocator = new Locator(LocatorParams.TYPE_SELECTOR);
	...
	private Locator findLocatorFromButtons(Element element) {
		for (PageButton button : pageButtons) {
			if (button.getElement().isOrHasChild(element)) {
				...
				pageButtonLocator.setParameter(LocatorParams.PARAM_NUM, String.valueOf(button.getNumber()));
				return pageButtonLocator;
			}
		}
		return null;
	...
	private Element getSelectorElement(Locator locator) {
		...
		} else if (LocatorParams.TYPE_ITEM.equals(cpecificPart.getType()) && selector.isExpanded()) {
