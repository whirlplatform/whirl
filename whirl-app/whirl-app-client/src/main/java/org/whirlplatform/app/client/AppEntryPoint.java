...
public class AppEntryPoint implements EntryPoint {
	...
	/**
	 * Получить приложение. <br>
	 * 1. Получить пользователя из сесcии.<br>
	 * 2. Выбрать приложение (текущее или гостевое в зависимости от значения
	 * guestURL).<br>
	 * 3. Получить корневой компонент приложения, начать загрузку
	 */
		...
		DataServiceAsync.Util.getDataService(new AsyncCallback<ApplicationData>() {
			...
			public void onFailure(Throwable caught) {
				...
				if (caught instanceof ClientRestException && ((ClientRestException) caught).getData() != null
                        && ((ClientRestException) caught).getData().getType() == ExceptionData.ExceptionType.WRONGAPP) {
	...
	/**
	 * Проверяет соотвествие кода полученного приложения коду в адресной строке
	 * браузера. При не совпадении перезагружает страницу с подставлением в
	 * адресную страку полученного кода.
	 * 
	 * @param application
	 *            {@link ApplicationData}
	 * @return <code>true</code> - продолжать загрузку приложения,
	...
	private static String getApplicationCode() {
		return Location.getParameter(AppConstant.ROLE_URL);
	}
	...
	private static void buildApplication(final ApplicationData data) {
		...
		DataServiceAsync.Util.getDataService(callback).getComponents(SessionToken.get(), data.getRootComponentId());
	}
