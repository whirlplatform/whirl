...
public abstract class Panel extends Widget implements HasWidgets.ForIsWidget {
     ...
     * </p>
     * <p>
     * There are several important things that must take place in the correct order to properly add
     * or insert a Widget to a Panel. Not all of these steps will be relevant to every Panel, but
     * all of the steps must be considered.
     * <ol>
     ...
     * </p>
     * <p>
     * There are several important things that must take place in the correct order to properly
     ...
     * be called after physical attachment in the DOM is complete. This Panel becomes the parent of
     * the child Widget, and the child will now fire its {@link Widget#onAttach()} event if this
     * Panel is currently attached.
     *
     ...
     * Widget's parent is cleared. This method should be called after verifying that the child
     * Widget is an existing child of the Panel, but before physically removing the child Widget
     * from the DOM. The child will now fire its {@link Widget#onDetach()} event if this Panel is
     * currently attached.
     * </p>
     * <p>
     * Calls to {@link #orphan(Widget)} should be wrapped in a try/finally block to ensure that the
