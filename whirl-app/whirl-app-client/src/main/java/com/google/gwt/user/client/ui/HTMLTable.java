...
/**
 * HTMLTable contains the common table algorithms for {@link com.google.gwt.user.client.ui.Grid} and
...
public abstract class HTMLTable extends Panel implements SourcesTableEvents,
    ...
    protected ElementMapperImpl<Widget> widgetMap = new ElementMapperImpl<Widget>();
    /**
     ...
     * @deprecated add a click handler instead and use {@link HTMLTable#getCellForEvent(ClickEvent)}
     * to get the cell information (remember to check for a null return value)
     */
    @Deprecated
    ...
    /**
     * Removes all widgets from this table, but does not remove other HTML or text contents of
     * cells.
     */
    @Override
    public void clear() {
    ...
    /**
     * Removes all widgets from this table, optionally clearing the inner HTML of each cell.  Note
     * that this method does not remove any cells or rows.
     *
    ...
    /**
     * Gets the {@link CellFormatter} associated with this table. Use casting to get
    ...
    public boolean isCellPresent(int row, int column) {
        ...
        return (column >= 0) && (column < getCellCount(row));
    }
     ...
     * @deprecated Use the {@link HandlerRegistration#removeHandler} method on the object returned
     * by an add*Handler method instead
     */
    @Deprecated
    ...
    /**
     * Sets the width of the table's border. This border is displayed around all cells in the
     * table.
     *
     ...
     * <p>
     * Inherited implementations may either throw IndexOutOfBounds exception if the cell does not
     * exist, or allocate a new cell to store the content.
     * </p>
     * <p>
     * FlexTable will automatically allocate the cell at the correct location and then set the
     ...
     * internalClearCell rather than clearCell, as clearCell may be overridden in subclasses to
    ...
    /**
     * Subclasses can implement this method. It allows them to decide what to do just before a
     * column is accessed. For classes, such as
    ...
    /**
     * Subclasses must implement this method. If the row already exists, this method must do
     * nothing. Otherwise, a subclass must either ensure that the row exists or throw an
    ...
    public class CellFormatter {
        ...
        /**
         * Sets whether this cell is visible via the display style property. The other cells in the
         * row will all shift left to fill the cell's space. So, for example a table with (0,1,2)
         * will become (1,2) if cell 1 is hidden.
         *
         * @param row     the row of the cell whose visibility is to be set
         * @param column  the column of the cell whose visibility is to be set
        ...
        /**
         * Gets the element associated with a cell. If it does not exist and the subtype allows
         * creation of elements, creates it.
         *
         * @param row    the cell's row
         * @param column the cell's column
         * @return the cell's element
        ...
        /**
         * Gets the TD element representing the specified cell unsafely (meaning that it doesn't
         * ensure that <code>row</code> and <code>column</code> are valid).
         *
         * @param row    the row of the cell to be retrieved
         * @param column the column of the cell to be retrieved
    ...
    public class ColumnFormatter {
        ...
        /**
         * Prepare the colgroup tag for the first time, guaranteeing that it exists and has at least
         * one col tag in it. This method corrects a Mozilla issue where the col tag will affect the
         * wrong column if a col tag doesn't exist when the element is attached to the page.
         */
    ...
    public class RowFormatter {
        ...
        /**
         * Ensure the TR element representing the specified row exists for subclasses that allow
