...
public class SQLConditionTest {
    ...
    public void cloneTest() {
        ...
        Assert.assertEquals("SQLCondition origin value was not set", ORIGIN_VALUE, origin.getValue());
        SQLCondition clone = origin.clone();
        Assert.assertEquals("SQLCondition clone value was not copied", ORIGIN_VALUE, clone.getValue());
        clone.setValue(CLONE_VALUE);
        Assert.assertEquals("SQLCondition clone value was not set", CLONE_VALUE, clone.getValue());
        Assert.assertEquals("SQLCondition origin value depends on clone value", ORIGIN_VALUE, origin.getValue());
    }
    ...
    public void equalsTest() {
        ...
        clone.setValue(CLONE_VALUE);
        Assert.assertEquals("Conditions with the same value do not equal to each other", origin, origin2);
