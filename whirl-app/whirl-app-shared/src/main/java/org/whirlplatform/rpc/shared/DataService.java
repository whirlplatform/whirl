...
public interface DataService {
    ...
    @POST
    @Path("/getUser")
    ClientUser getUser(@FormParam("token") SessionToken token,
                       @FormParam("newSession") boolean newSession,
    ...
    ApplicationData getApplication(@FormParam("token") SessionToken token,
                                   @FormParam("applicationCode") String applictionCode,
                                   @FormParam("version") Version version,
                                   @FormParam("locale") String locale);

    @POST
    @Path("/getComponents")
    ComponentModel getComponents(@FormParam("token") SessionToken token,
                                 @FormParam("appId") String appId);

    @POST
    @Path("/getTableConfig")
    TableConfig getTableConfig(@FormParam("token") SessionToken token,
                               @FormParam("classId") String classId,
                               @FormParam("whereSql") String whereSql,
                               @FormParam("params") Map<String, DataValue> params);
    ...
    @POST
    @Path("/insert")
    RowModelData insert(@FormParam("token") SessionToken token,
                        @FormParam("metadata") ClassMetadata metadata,
                        @FormParam("config") DataModifyConfig config);

    @POST
    @Path("/update")
    RowModelData update(@FormParam("token") SessionToken token,
                        @FormParam("metadata") ClassMetadata metadata,
                        @FormParam("config") DataModifyConfig config);

    @POST
    @Path("/delete")
    void delete(@FormParam("token") SessionToken token,
                @FormParam("metadata") ClassMetadata metadata,
                @FormParam("config") DataModifyConfig config);
    ...
    @POST
    @Path("getNextEvent")
    EventMetadata getNextEvent(@FormParam("token") SessionToken token,
                               @FormParam("event") EventMetadata event,
                               @FormParam("nextEventCode") String nextEventCode);

    @POST
    @Path("executeServer")
    EventResult executeServer(@FormParam("token") SessionToken token,
                              @FormParam("event") EventMetadata event,
                              @FormParam("parameters") ListHolder<DataValue> parameters);

    @POST
    @Path("getReportFields")
    List<FieldMetadata> getReportFields(@FormParam("token") SessionToken token,
                                        @FormParam("classId") String classId);

    @POST
    @Path("saveReportValues")
    void saveReportValues(@FormParam("token") SessionToken token,
                          @FormParam("componentId") String componentId,
                          @FormParam("values") Map<String, DataValue> values);

    @POST
    @Path("getEvent")
    EventMetadata getEvent(@FormParam("token") SessionToken token,
    ...
    Boolean checkCaptchaCode(@FormParam("token") SessionToken token,
                             @FormParam("captchaCode") String captchaCode,
                             @FormParam("componentId") String componentId);

    @POST
    @Path("getFreeEvent")
    EventMetadata getFreeEvent(@FormParam("token") SessionToken token,
                               @FormParam("eventCode") String eventCode);

    @POST
    @Path("saveLoadConfig")
    @Produces(MediaType.TEXT_PLAIN)
    String saveLoadConfig(@FormParam("token") SessionToken token,
                          @FormParam("loadConfig") ClassLoadConfig loadConfig);
