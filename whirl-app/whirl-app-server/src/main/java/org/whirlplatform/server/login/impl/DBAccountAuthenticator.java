...
public class DBAccountAuthenticator implements AccountAuthenticator {
	...
	public ApplicationUser login(LoginData login) throws LoginException, CustomException {
		try (ConnectionWrapper connection = connectionProvider.getConnection(SrvConstant.DEFAULT_CONNECTION)) {
			...
			command.select(db.WHIRL_USERS.ID, loginCol, db.WHIRL_USERS.PASSWORD_HASH, db.WHIRL_USERS.NAME,
			...
			if (reader.moveNext() && login.getLogin().equalsIgnoreCase(reader.getString(loginCol))
					&& BCrypt.checkpw(login.getPassword(), reader.getString(db.WHIRL_USERS.PASSWORD_HASH))) {
