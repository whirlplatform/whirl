...
public class TableWhirlUserGroups extends EmpireDBTable {
    ...
    public final DBTableColumn GROUP_CODE;

    public TableWhirlUserGroups(MetadataDatabase db) {
        super("WHIRL_USER_GROUPS", db);
        ...
        GROUP_CODE = addColumn("GROUP_CODE", DataType.TEXT, 255.0, false, null);

        // configure key columns (primary key)
        setPrimaryKey(ID);

        addIndex("IDX_WHIRL_USER_GROUPS_RWU", false, R_WHIRL_USERS);
