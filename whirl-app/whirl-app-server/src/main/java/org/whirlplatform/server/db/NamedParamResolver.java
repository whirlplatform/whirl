...
public class NamedParamResolver {
	...
	private String template(String sql, Map<String, DataValue> params) {
		...
		for (Entry<String, DataValue> e : params.entrySet()) {
			...
			} else if (e.getValue() instanceof RowListValue && isRowListEmpty((RowListValue) e.getValue())) {
	...
	private boolean isRowListEmpty(RowListValue r) {
		if (r == null || r.getRowList().size() == 0) {
			return true;
        } else return r.isCheckable() && r.getRowList().size() == 1 && !r.getRowList().get(0).isChecked();
	}
	...
	private String parse(String sql) {
		...
		for (int i = 0; i < sql.length(); i++) {
			...
			if (inSingleQuote) {
				...
				if ("in".equalsIgnoreCase(inWord.trim()) && sql.substring(i + 1).trim().startsWith(":")) {
					inInClause = true;
				}
			} else if (":".equals(c) && i + 1 < sql.length() && Character.isJavaIdentifierPart(sql.charAt(i + 1))) {
				int j = i + 2;
				while (j < sql.length() && Character.isJavaIdentifierPart(sql.charAt(j)))
					j++;
	...
	private String escapeParameter(DataValue parameter) {
		...
		case BOOLEAN:
			result = driver.getValueString(parameter.getBoolean(), org.apache.empire.data.DataType.BOOL);
			break;
		case DATE:
			result = driver.getValueString(parameter.getDate(), org.apache.empire.data.DataType.DATETIME);
			break;
		case NUMBER:
			result = driver.getValueString(parameter.getDouble(), org.apache.empire.data.DataType.FLOAT);
			break;
		case STRING:
			result = driver.getValueString(parameter.getString(), org.apache.empire.data.DataType.TEXT);
			break;
		case LIST:
			...
			break;
		default:
			result = driver.getValueString(parameter.getString(), org.apache.empire.data.DataType.UNKNOWN);
			break;
		}
		return result;
	}
