...
public class DBConnection {
	...
	/**
	 * <b>ВНИМАНИЕ!<br>
	 * Метод не должен быть использован для получения соединений. Соединения
	 * необходимо получать через {@link ConnectionProvider} который можно получить
	 * через внедрение зависимостей. Метод оставлен публичным для возможности
	 * использования в {@link LoggerFactory}. </b><br>
	 * <br>
	 ...
	 * Метод не должен быть использован для получения соединений. Соединения
	 * необходимо получать через {@link ConnectionProvider} который можно получить
	 * через внедрение зависимостей. </b><br>
	 * <br>
	 * Коннект к БД - получение коннекта из пула
	 *
	 * @throws ConnectException
	 * @throws IOException
	 */
	static ConnectionWrapper getConnection(String alias, ApplicationUser user) throws ConnectException {
		...
		try {
			...
			Constructor<ConnectionWrapper> constructor = clazz.getConstructor(String.class, Connection.class,
			...
			_log.error(I18NMessage.getMessage(I18NMessage.getRequestLocale()).errorDBConnect() + ": " + alias, e);
		}
		if (connection == null) {
			throw new ConnectException(I18NMessage.getMessage(I18NMessage.getRequestLocale()).errorDBConnect());
		}
		return connection;
	}

	@SuppressWarnings("unchecked")
	private static Class<ConnectionWrapper> connectionWrapperClass(String alias) throws ConnectException {
		try {
            String driverClassName = ContextUtil.lookup("Whirl/ds/" + alias + "/driver");
			Class<ConnectionWrapper> clazz = (Class<ConnectionWrapper>) Class.forName(driverClassName);
			return clazz;
