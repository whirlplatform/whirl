...
public class DataServiceImpl implements DataService, DirectRestService {
	...
	/**
	 * По идее jersey оборачивает объект в прокси, и в каждом потоке будет свой
	 * HttpServletRequest
	 */
	@Context
	...
	@Override
	public ApplicationData getApplication(SessionToken token, String applicationCode, Version version, String locale) {
	...
	public FormModel getForm(SessionToken token, String formId, ListHolder<DataValue> parameters) {
		FormModel form = connector().getForm(formId, parameters.getList(), getApplicationUser(token));
		return form;
	}
	...
	private EventResult executeJava(EventMetadata metadata, List<DataValue> parameters, ApplicationUser user) {
		...
		RunningEvent ev = new RunningEvent(RunningEvent.Type.JAVAEVENT, metadata.getCode(), "", user.getLogin()) {
			@Override
			public void onStop() {
		...
		try (Profile p = new ProfileImpl(msg, ev)) {
			JavaExecutor exec = new JavaExecutor(connector(), connectionProvider, user, metadata.getSource(),
					servletContext);
	...
	public Boolean checkCaptchaCode(SessionToken token, String captchaCode, String componentId) { // ajax-проверка
