...
public class EditorHelper {
	...
	/**
	 * Позволяет указать один обработчик нескольким компонентам.<br />
	 * Для каждого компонента создаётся обработчик своего типа <br />
	 * (SelectHandler для TextButton, KeyDown(ENTER) для TextField и
	 * PasswordField).<br />
	 * Используется для формы логина, формируемой компонентами GXT (не HTML
	 * &lt;form&gt;).<br />
	 * 
	 * @param h
	 *            Команда обработки
	 * @param widgets
	 *            Компоненты типов из множества [TextButton, TextField,
	 *            PasswordField]
	 */
