...
public class PropertyFormView extends AccordionLayoutContainer
	...
	private void initRowsPanel() {
		...
		final CheckBoxSelectionModel<RowElement> sm = new CheckBoxSelectionModel<RowElement>(identity);
		...
		ColumnConfig<RowElement, Double> cc1 = new ColumnConfig<RowElement, Double>(props.height(), 100,
	...
	private void initColsPanel() {
		...
		CheckBoxSelectionModel<ColumnElement> sm = new CheckBoxSelectionModel<ColumnElement>(identity);
		sm.setSelectionMode(SelectionMode.SINGLE);
		...
		List<ColumnConfig<ColumnElement, ?>> columns = new ArrayList<ColumnConfig<ColumnElement, ?>>();
		columns.add(sm.getColumn());
		columns.add(new ColumnConfig<ColumnElement, Integer>(props.column(), 100,
				EditorMessage.Util.MESSAGE.property_form_column_index()));
		ColumnConfig<ColumnElement, Double> cc1 = new ColumnConfig<ColumnElement, Double>(props.width(), 100,
	...
	private void initRequestsPanel() {
		...
		IdentityValueProvider<RequestElement> identity = new IdentityValueProvider<RequestElement>();
		final CheckBoxSelectionModel<RequestElement> sm = new CheckBoxSelectionModel<RequestElement>(identity);
		sm.setSelectionMode(SelectionMode.SINGLE);
		...
		List<ColumnConfig<RequestElement, ?>> columns = new ArrayList<ColumnConfig<RequestElement, ?>>();
		columns.add(sm.getColumn());
		ColumnConfig<RequestElement, String> cc1 = new ColumnConfig<RequestElement, String>(props.sql(), 100,
				EditorMessage.Util.MESSAGE.property_form_sql_query());
		ColumnConfig<RequestElement, Integer> cc2 = new ColumnConfig<RequestElement, Integer>(props.top(), 50,
				EditorMessage.Util.MESSAGE.property_form_sql_top_index());
		ColumnConfig<RequestElement, Integer> cc3 = new ColumnConfig<RequestElement, Integer>(props.bottom(), 50,
				EditorMessage.Util.MESSAGE.property_form_sql_bottom_index());
		ColumnConfig<RequestElement, PropertyValue> cc4 = new ColumnConfig<RequestElement, PropertyValue>(
				props.emptyText(), 100, EditorMessage.Util.MESSAGE.property_form_sql_no_data());
		ColumnConfig<RequestElement, DataSourceElement> cc5 = new ColumnConfig<RequestElement, DataSourceElement>(
				props.dataSource(), 100, EditorMessage.Util.MESSAGE.property_form_sql_datasource());
		columns.add(cc1);
		columns.add(cc2);
		columns.add(cc3);
		columns.add(cc4);
		...
		editing.addEditor(cc2, new NumberField<Integer>(new NumberPropertyEditor.IntegerPropertyEditor(null)));
		editing.addEditor(cc3, new NumberField<Integer>(new NumberPropertyEditor.IntegerPropertyEditor(null)));
		editing.addEditor(cc4, new PropertyValueFieldConverter(requestNoDataField), requestNoDataField);
		editing.addEditor(cc5, new DataSourceComboBox(new DataSourceStore(getPresenter().getEventBus())));
		grid.setSelectionModel(sm);
		SelectHandler sh1 = new SelectHandler() {
			...
			public void onSelect(SelectEvent event) {
				...
				if (selectedCells != null) {
					boolean duplicate = false;
					for (RequestElement m : reqsStore.getAll()) {
						if (selectedCells.getTop() == m.getTop() && selectedCells.getBottom() == m.getBottom()) {
							duplicate = true;
							break;
						}
					}
