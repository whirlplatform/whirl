...
public class ApplicationsTreeGrid extends TreeGrid<ApplicationStoreData> {
	 ...
	 */
	public abstract static class ReadOnlyValueProvider implements ValueProvider<ApplicationStoreData, String> {
	...
	private static ColumnConfig<ApplicationStoreData, String> treeColumn = new ColumnConfig<>(createNameValueProvider(),
	...
	public ApplicationsTreeGrid() {
		...
		treeColumn.setCell(new AbstractCell<String>() {
			...
			public void render(com.google.gwt.cell.client.Cell.Context context, String value, SafeHtmlBuilder sb) {
				ApplicationStoreData data = getTreeStore().findModelWithKey(context.getKey().toString());
	...
	public static int estimatedWidth() {
		return MODIFIED_COLUMN_WIDTH + NAME_COLUMN_WIDTH + TITLE_COLUMN_WIDTH + REVISION_COLUMN_WIDTH
	 ...
	 * 
	 * @return selected item or null if it is not a ApplicationStoreData
	 *         object dummy folder
	 */
	 ...
	 * 
	 * @param data
	 *            - the collection of ApplicationStoreData items
	 */
	...
	/**
	 * Collects ApplicationStoreData for the applications containing given
	 * code
	 * 
	 * @param code
	 * @return The collection of the ApplicationStoreData objects
	...
	private static TreeStore<ApplicationStoreData> createTreeStore() {
		TreeStore<ApplicationStoreData> result = new TreeStore<>(new ModelKeyProvider<ApplicationStoreData>() {
			@Override
			public String getKey(ApplicationStoreData item) {
				return (item == null) ? null : item.getId();
			}
		});
		result.addSortInfo(new StoreSortInfo<ApplicationStoreData>(new Comparator<ApplicationStoreData>() {
			@Override
			public int compare(ApplicationStoreData o1, ApplicationStoreData o2) {
				int compareResult = (o1.getCode() != null) ? o1.getCode().compareToIgnoreCase(o2.getCode()) : -1;
				if (compareResult == 0) {
	...
	private static ColumnModel<ApplicationStoreData> createColumnModel() {
		ColumnConfig<ApplicationStoreData, String> modifiedColumn = new ColumnConfig<>(createModifiedValueProvider(),
				MODIFIED_COLUMN_WIDTH, MODIFIED_CAPTION);
		ColumnConfig<ApplicationStoreData, String> titleColumn = new ColumnConfig<>(createTitleValueProvider(),
