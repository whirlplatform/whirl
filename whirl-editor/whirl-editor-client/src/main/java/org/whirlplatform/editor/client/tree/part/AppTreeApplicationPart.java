...
public class AppTreeApplicationPart extends AbstractAppTreePart<ApplicationElement> {
    ...
    public AppTreeApplicationPart(AppTree appTree, AppTreePresenter treePresenter, ApplicationElement application) {
    ...
    public boolean isAdding(AbstractElement element) {
        return element == folders.events || element == folders.datasources || element == folders.references
    ...
    public boolean hasRights(AbstractElement element) {
        return element == folders.components || element == folders.freeComponents || element == folders.events
    ...
    public boolean isDeleting(AbstractElement element) {
        return element == handledElement.getRootComponent()
                || (element instanceof ComponentElement && handledElement.getFreeComponents().contains(element))
                || (element instanceof EventElement && handledElement.getFreeEvents().contains(element))
                || (element instanceof DataSourceElement && handledElement.getDataSources().contains(element));
    }
    ...
    public boolean doAddElement(AbstractElement parent, AbstractElement element) {
        if (parent == folders.events && element == null) {
            ...
            final AsyncCallback<ApplicationElement> callback = new AsyncCallback<ApplicationElement>() {
                @Override
            ...
            treePresenter.riseShowOpenApplicationsCallback(new Callback<ApplicationStoreData, Throwable>() {
                @Override
                ...
                public void onSuccess(ApplicationStoreData result) {
                    EditorDataService.Util.getDataService().loadApplication(result, callback);
                }
            });
            return true;
    ...
    public boolean doAddElementUI(AbstractElement parent, AbstractElement element) {
        if ((parent == handledElement || parent instanceof NullRootComponentElement)
            ...
            putTreePart(element, new AppTreeComponentPart(appTree, treePresenter, (ComponentElement) element));
            return true;
            ...
            putTreePart(element, new AppTreeDataSourcePart(appTree, treePresenter, (DataSourceElement) element));
            return true;
            ...
            putTreePart(element, new AppTreeReferencePart(appTree, treePresenter, (ApplicationElement) element));
            return true;
        } else if (parent == handledElement && element instanceof EventElement) {
            removeElement(element);
            addChildElement(folders.events, element);
    ...
    public boolean doRemoveElement(AbstractElement parent, AbstractElement element) {
        if (handledElement.getRootComponent() == element) {
            ...
            return true;
        } else if (element instanceof ComponentElement && handledElement.getFreeComponents().contains(element)) {
            treePresenter.riseRemoveElement(new NullFreeComponentElement(), element, true);
            return true;
        } else if ((element instanceof EventElement && handledElement.getFreeEvents().contains(element))
                || (element instanceof DataSourceElement && handledElement.getDataSources().contains(element))) {
            treePresenter.riseRemoveElement(handledElement, element, true);
            return true;
        } else if (element instanceof ApplicationElement && handledElement.getReferences().contains(element)) {
            treePresenter.riseRemoveElement(handledElement, element, true);
            return true;
        }
        return false;
    ...
    public boolean doRemoveElementUI(AbstractElement parent, AbstractElement element) {
        /* компонент */
        if ((element instanceof ComponentElement && (appTree.hasChild(folders.components, element, false)
                || appTree.hasChild(folders.freeComponents, element, false)))
                /* событие */
                || (element instanceof EventElement && appTree.hasChild(folders.events, element, false))
                /* источник данных */
                || (element instanceof DataSourceElement && appTree.hasChild(folders.datasources, element, false))
                /* зависимости */
                || (element instanceof ApplicationElement && appTree.hasChild(folders.references, element, false))) {
            removeElement(element);
            return true;
        }
        return false;
    ...
    public boolean doEditElementRights(AbstractElement element) {
        if (element == folders.events) {
            if (!handledElement.getFreeEvents().isEmpty()) {
                treePresenter.riseEditRights(handledElement.getFreeEvents(), Collections.singleton(RightType.EXECUTE));
            }
            return true;
        } else if (element == folders.components) {
            Collection<EventElement> events = getAllComponentsEvents(handledElement.getRootComponent());
            if (!events.isEmpty()) {
            ...
            if (!tables.isEmpty()) {
                treePresenter.riseEditRights(tables, Collections.unmodifiableCollection(Arrays.asList(RightType.ADD,
                        RightType.DELETE, RightType.EDIT, RightType.VIEW, RightType.RESTRICT)));
            }
        }
        return false;
    }
    ...
    public boolean canDragDrop(AbstractElement dropTarget, Object dropData) {
        if ((dropData instanceof ComponentType || dropData instanceof ComponentElement)
            ...
            return true;
                ...
                && (ComponentType.FormBuilderType.equals(dropData) || dropData instanceof FormElement)) {
            return true;
    ...
    public boolean doDragDrop(AbstractElement dropTarget, Object dropData) {
        if (dropTarget == folders.freeComponents && dropData instanceof ComponentType) {
            ...
            treePresenter.riseAddElement(new NullFreeComponentElement(), (ComponentElement) dropData);
            ...
            treePresenter.riseAddElement(new NullRootComponentElement(), (ComponentElement) dropData);
        } else if (dropTarget instanceof ReportElement && ComponentType.FormBuilderType.equals(dropData)) {
            treePresenter.riseAddElement(dropTarget, new NewComponentElement((ComponentType) dropData));
            return true;
    ...
    private boolean isComponentAddable(AbstractElement element) {
        if (element instanceof ComponentElement) {
            ...
            return type.equals(ComponentType.HorizontalMenuType) || type.equals(ComponentType.ButtonType)
                    || type.equals(ComponentType.HorizontalMenuItemType) || type.equals(ComponentType.TreeMenuType);
        }
        return false;
    }
}
