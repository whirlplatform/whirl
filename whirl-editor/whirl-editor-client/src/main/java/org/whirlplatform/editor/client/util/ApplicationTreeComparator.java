...
public class ApplicationTreeComparator implements Comparator<AbstractElement> {
    ...
    public int compare(AbstractElement first, AbstractElement second) {
        if (first instanceof ComponentElement && second instanceof ComponentElement) {
        ...
        } else if (first instanceof EventParameterElement && second instanceof EventParameterElement) {
            return ((EventParameterElement) first).getIndex() - ((EventParameterElement) second).getIndex();
        } else if (first instanceof AbstractTableElement && second instanceof AbstractTableElement) {
            return (first != null && first.getName() != null) ? first.getName().compareTo(second.getName()) : -1;
            ...
            return (first != null && ((ApplicationElement) first).getCode() != null)
                    ? ((ApplicationElement) first).getCode().compareTo(((ApplicationElement) second).getCode()) : -1;
        ...
        } else if (first instanceof ContextMenuItemElement && second instanceof ContextMenuItemElement) {
            return ((ContextMenuItemElement) first).getIndex() - ((ContextMenuItemElement) second).getIndex();
        } else if (first instanceof ContextMenuItemElement && !(second instanceof ContextMenuItemElement)) {
            return -1;
        } else if (second instanceof ContextMenuItemElement && !(first instanceof ContextMenuItemElement)) {
            return 1;
        }
        return (first != null && first.getName() != null) ? first.getName().compareTo(second.getName()) : -1;
    }
    ...
    private int compareComponents(ComponentElement first, ComponentElement second) {
        int res = 0;
        if (first.getParent() != null && second.getParent() != null && first.getParent() == second.getParent()) {
            if (first.getParent().getType() == ComponentType.VerticalContainerType
                ...
                int index1 = parseIndex(String.valueOf(first.getProperty(PropertyType.LayoutDataIndex)
                        .getValue(first.getProperty(PropertyType.LayoutDataIndex).getDefaultLocale()).getInteger()));
                int index2 = parseIndex(String.valueOf(second.getProperty(PropertyType.LayoutDataIndex)
                        .getValue(second.getProperty(PropertyType.LayoutDataIndex).getDefaultLocale()).getInteger()));
                res = index1 - index2;
            } else if (first.getParent().getType() == ComponentType.BorderContainerType) {
                int index1 = parseLocation(String.valueOf(first.getProperty(PropertyType.LayoutDataLocation)
                        .getValue(first.getProperty(PropertyType.LayoutDataLocation).getDefaultLocale()).getInteger()));
                int index2 = parseLocation(String.valueOf(second.getProperty(PropertyType.LayoutDataLocation)
                        .getValue(second.getProperty(PropertyType.LayoutDataLocation).getDefaultLocale()).getInteger()));
                res = index1 - index2;
            } else if (first.getParent().getType() == ComponentType.FormBuilderType) {
                int cols = first.getParent().getProperty(PropertyType.Columns)
                        .getValue(first.getParent().getProperty(PropertyType.Columns).getDefaultLocale()).getInteger();
                int row1 = first.getProperty(PropertyType.LayoutDataFormRow)
                        .getValue(first.getProperty(PropertyType.LayoutDataFormRow).getDefaultLocale()).getInteger();
                int col1 = first.getProperty(PropertyType.LayoutDataFormColumn)
                        .getValue(first.getProperty(PropertyType.LayoutDataFormColumn).getDefaultLocale()).getInteger();
                int row2 = second.getProperty(PropertyType.LayoutDataFormRow)
                        .getValue(second.getProperty(PropertyType.LayoutDataFormRow).getDefaultLocale()).getInteger();
                int col2 = second.getProperty(PropertyType.LayoutDataFormColumn)
                        .getValue(second.getProperty(PropertyType.LayoutDataFormColumn).getDefaultLocale()).getInteger();
