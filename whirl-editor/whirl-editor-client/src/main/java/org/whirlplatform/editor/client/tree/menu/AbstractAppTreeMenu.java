 ...
 * <p>
 * Перед открытием атоматически обновляет состояние всех входящих в его состав AppTreeMenuItem.
 * Логика измененения состояния пунктов меню скрыта внутри их реализации. Состояние пунктов меню не
 * являющимися AppTreeMenuItem не затрагивается. Добавление и удаление AppTreeMenuItem производится
 * стандартным способом через вызов add или remove.
 *
 * @see AppTreeMenuItem
 */
public class AbstractAppTreeMenu<T extends AppTree> extends Menu implements AppTreeMenu<T> {
    ...
    /**
     * Устанавливает ссылку на дерево приложения как для самого меню, так и для всех его пунктов
     * типа AppTreeMenuItem.
     *
