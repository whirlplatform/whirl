 ...
 * <p>
 * Использование как стандартной GXT кнопки с помощью метода asTextButton. Предполагается что при
 * реализации поведения и изменения состояния Контекстной кнопки будут использованы только методы
...
public interface ContextTextButton<C> extends HasContext<C>, HasUpdatableState {
    ...
    /**
     * Возвращает GXT представление. Позволяет добавить Контекстную кнопку как стандартный GXT
     * элемент TextButton.
     *
