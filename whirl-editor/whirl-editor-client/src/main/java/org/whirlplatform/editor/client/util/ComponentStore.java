...
public class ComponentStore extends ListStore<ComponentElement> {
    ...
    private static final Comparator<ComponentElement> comparator = new Comparator<ComponentElement>() {
    ...
    private class ComponentProxy implements
        ...
        public void load(final String loadConfig,
            ...
            provider.getAvailableComponents(new Callback<Collection<ComponentElement>, Throwable>() {
                @Override
                ...
                public void onSuccess(Collection<ComponentElement> result) {
                    ...
                    if (loadConfig == null || loadConfig.isEmpty()) {
                        ...
                        for (ComponentElement element : result) {
                            ...
                            String val = element.getProperty(PropertyType.Code).getDefaultValue().getString();
                            boolean isCode = val != null && (String.valueOf(val).toLowerCase().contains(query));
                            if (isName || isCode) {
                                if (containersOnly) {
                                    if (element.getType().isContainer()) {
                                        list.add(element);
