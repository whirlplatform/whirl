...
import org.whirlplatform.editor.client.util.*;
...
public class PropertyEditorView extends AccordionLayoutContainer implements
		IPropertyEditorView {
	...
	private class ToggleWidget extends Widget implements HasClickHandlers {
		...
		private ToggleWidget(Widget widget) {
			img = DOM.createImg();
			img.setAttribute("src", EditorBundle.INSTANCE.greenToggle()
					.getSafeUri().asString());
		...
		private void setReplaceable(boolean replaceable) {
			if (replaceable) {
				img.setAttribute("src", EditorBundle.INSTANCE.redToggle()
						.getSafeUri().asString());
			} else {
				img.setAttribute("src", EditorBundle.INSTANCE.greenToggle()
						.getSafeUri().asString());
	...
	public void initPropertiesPanel() {
		...
		propertiesContainer.getElement().getStyle()
	...
	public void initLayoutPanel() {
		...
		layoutPanel
				.addValueChangeHandler(new ValueChangeHandler<PropertyType>() {

					@Override
					public void onValueChange(
							ValueChangeEvent<PropertyType> event) {
						PropertyValue value = layoutPanel.getProperty(event
								.getValue());
						presenter.onChangeComponentProperty(event.getValue(),
								defaultLocale, false, value.getValue(null)
										.getValue());
					}
				});
	}
	...
	public class CompositeCell extends HorizontalLayoutContainer {
		...
		public void setValue(PropertyValue value) {
			...
			if (value.isReplaceable()) {
				if (PropertyType.Parameters == propertyType) {
					((ParameterEditorComponent) typeField).setValue(dataValue
							.getString());
				} else {
					for (LocaleElement locale : value.getLocales()) {
						textField.setValue(
								locale, value.getValue(locale)
										.getString());
					}

					LocaleElement key = value.getDefaultLocale() != null ? value
							.getDefaultLocale()
							: textField
									.getCurrentKey();
			...
			if (PropertyType.Parameters == propertyType) {
				...
				AbstractElement e = ((ComboBox<AbstractElement>) typeField)
						.getStore().findModelWithKey(l.getId());
				...
				((NumberField<Double>) typeField).setValue(dataValue
						.getDouble());
			} else if (DataType.STRING.equals(dataValue.getType())
					) {
				if (typeField instanceof SimpleComboBox) {
					((SimpleComboBox<String>) typeField)
							.setValue( dataValue.getString());
				} else {
					for (LocaleElement locale : value.getLocales()) {
						((MultiSetField<LocaleElement>) typeField).setValue(
								locale, value.getValue(locale)
										.getString());
					}
					...
					LocaleElement key = value.getDefaultLocale() != null ? value
							.getDefaultLocale()
							: ((MultiSetField<LocaleElement>) typeField)
									.getCurrentKey();
		...
		private NumberField<Double> createDoubleField() {
			NumberField<Double> field = new NumberField<Double>(
		...
		private PropertyValueField createStringField() {
			PropertyValueField field = new PropertyValueField(
					GWT.create(MultiSetCellTinyResources.class));
			if (propertyType.isSimple()) {
				field.setLocales(defaultLocale,
		...
		private Field<? extends AbstractElement> createListField() {
			...
			if (propertyType == PropertyType.DataSource) {
				field = new TableComboBox(new TableStore(
				...
				((ComboBox<? extends AbstractElement>) field)
		...
		private Field<String> createParametersField() {
			return new ParameterEditorComponent(new ComponentStore(
					getPresenter().getEventBus(), false));
		}
		...
		private void createFields() {
			...
			add(label, new HorizontalLayoutData(0.5, -1,
					new Margins(3, 0, 0, 0)));
			...
			if (typeField instanceof ComboBox) {
						...
						.addSelectionHandler(new SelectionHandler<Object>() {
							...
							public void onSelection(SelectionEvent<Object> event) {
								...
								if (value instanceof AbstractElement) {
									if (((AbstractElement) value).getId()
											.isEmpty()) {
										value = null;
									} else {
										EditListModelDataImpl m = new EditListModelDataImpl();
										m.setId(((AbstractElement) value)
												.getId());
										m.setLabel(((AbstractElement) value)
												.getName());
										m.setElement((AbstractElement)value);
										value = m;
									}
								}
								presenter.onChangeComponentProperty(
										propertyType, l, false, value);
							}
						});
			} else {
				typeField.addValueChangeHandler(new ValueChangeHandler() {
					...
					public void onValueChange(ValueChangeEvent event) {
						...
						if (typeField instanceof MultiSetField) {
							l = ((MultiSetField<LocaleElement>) typeField)
									.getCurrentKey();
						}
			...
			textField.addValueChangeHandler(new ValueChangeHandler<String>() {
				...
				public void onValueChange(ValueChangeEvent<String> event) {
					presenter.onChangeComponentProperty(propertyType,
							textField.getCurrentKey(), true, event.getValue());
				}
			});
